...
plt.clf()
fig = plt.figure()
plt.plot(D+25,predictedprice,label='Ypredict',color='red')
plt.plot(d,ldfdata[:,0][0:100],label='Yactual')
plt.legend()                 
plt.xlabel('Days')
plt.ylabel('Prices')
plt.xlim(1,100)        # set x scale
plt.ylim(0,500)        # set y scale
plt.title('Ypredict vs Yatual for File 151',fontsize=12)

plt.savefig('Predicted vs actual 151.pdf',format='pdf')
plt.close()
print "The RMS of 151 first 100 days is:",math.sqrt(((predictedprice-ldfdata[:,0][25:100])**2).mean(axis=0))
print "The correlation of 151 first 100 days is:",np.corrcoef(predictedprice,ldfdata[:,0][25:100])[1,0]
